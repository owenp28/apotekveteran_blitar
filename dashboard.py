...
def modal_pilih_obat(df_source, initial_search=""):
    ...
    if not tampil_habis:
        df_modal = df_modal[df_modal["Stok Sisa"] > 0]
...
if "opname_custom_items" not in st.session_state:
                ...
                if edited_opname is not None and not edited_opname.empty:
                    for idx, row in edited_opname.iterrows():
                        ...
                        if nyata > 0 or stok_exp > 0:
                            ws_target = workbook_data[lokasi].copy()
                            ws_target = prepare_sheet_for_editor(ws_target)
...
"""
            ...
            lines_top = [
                "APOTEK VETERAN SEHAT BLITAR".center(W),
                "Jl. Veteran no 64B Blitar Kota".center(W),
                "(Sebelah Gang Srigading)".center(W),
                "081331808585".center(W),
                "-" * W
            ]
            ...
            with col_d_txt:
                ...
                <body>
                <button class="btn-txt" onclick="downloadTxtRealtime()">📄 Unduh Nota (Notepad .TXT)</button>
                <script>
                function downloadTxtRealtime() {{
                    ...
                    var infoKiri = tgl + " " + jam;
                    var infoKanan = "{kasir_nama_nota}";
                    var W = 32;
                    var spasiCount = Math.max(1, W - infoKiri.length - infoKanan.length);
                    var spasi = " ".repeat(spasiCount);
                    var metaLine = infoKiri + spasi + infoKanan;
                    
                    var txtContent = "{part_header}\\n" + metaLine + "\\n{part_body}";
                    
                    var blob = new Blob([txtContent], {{ type: "text/plain;charset=utf-8" }});
                    var link = document.createElement("a");
                    link.href = URL.createObjectURL(blob);
                    link.download = "Struk_Nota_" + now.getFullYear() + pad(now.getMonth() + 1) + pad(now.getDate()) + "_" + pad(now.getHours()) + pad(now.getMinutes()) + pad(now.getSeconds()) + ".txt";
                ...
                """
                components.html(txt_downloader_html, height=45)
